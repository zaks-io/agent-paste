---
name: agent-paste-goal-review-main-and-queue-fixes
description: Use when running the periodic agent-paste loop whose goal is to review new main-branch commits and queue fixes by filing actionable Linear issues for bugs, security regressions, or product-contract drift.
---

# Agent Paste Review Main And Queue Fixes
...
`${CODEX_HOME:-$HOME/.codex}/automation-state/agent-paste-goal-review-main-and-queue-fixes/last-reviewed-origin-main`,
...
- authorization shortcuts that bypass Workspace boundaries, scope checks,
  WorkOS/operator gates, RLS expectations, or content-origin isolation
...
Before creating a new issue, search existing AP issues for a matching problem,
...
- team: `AP`
- project: `agent-paste Roadmap`, using the closest milestone from
  `docs/agents/issue-tracker.md`
...
- optional labels: apply existing bug, tech-debt, risk, or area labels when the
  AP team already has them; do not invent non-canonical labels during this loop
...
- Treat review findings as queue input for
  `agent-paste-goal-keep-agent-queue-moving`; ordinary fixes should be handled
