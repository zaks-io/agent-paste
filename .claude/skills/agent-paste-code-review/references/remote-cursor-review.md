...
For the agent-paste repo, `.cursor/environment.json` uses `pnpm setup:codex --skip-env`; hosted secrets are opt-in per issue, so remote review should default to local checks only.
