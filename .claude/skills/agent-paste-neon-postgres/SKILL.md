---
name: agent-paste-neon-postgres
description: >-
...
This repo-local copy is named `agent-paste-neon-postgres` so agents can tell it
belongs to the `agent-paste` workspace. Upstream Neon install examples may still
refer to the source skill as `neon-postgres`.
