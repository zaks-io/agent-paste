---
name: agent-paste-implement-issue
description: Use when implementing one agent-paste Linear issue as one scoped branch and PR, especially a Todo or In Progress AP issue labeled ready-for-agent.
---

# Agent Paste Implement Issue
...
- include `AP-<number>` in the branch name when no branch was assigned
...
Before opening a PR, run or request `agent-paste-local-code-review` on the local
...
draft. Use `agent-paste-create-pr` when available.
