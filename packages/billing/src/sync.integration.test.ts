...
describe("applyBillingSnapshot (webhook semantics)", () => {
  ...
  it("activates pro and is an idempotent no-op on replay of the same event", async () => {
    ...
    expect(first).toMatchObject({ applied: true, plan: "pro", plan_changed: true });

    const replay = await applyBillingSnapshot({
      executor,
      actorId: "stripe_webhook",
      workspaceId,
      snapshot: snapshot(),
    ...
    // Same idempotency key (subscription + status + period end) returns the stored
    // first-call result verbatim and performs no second write.
