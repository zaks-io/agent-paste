...
describe("access link denylist invalidation", () => {
  it("writes ald: keys for revocation with the max content-token TTL", async () => {
    ...
    await expect(writeAccessLinkRevocationDenylist({ DENYLIST: { put } }, "al_1")).resolves.toBe(true);
    expect(put).toHaveBeenCalledWith("ald:al_1", expect.stringContaining('"reason":"revocation"'), {
  ...
  it("writes ad: keys for access-link lockdown", async () => {
    const put = vi.fn(async () => {});
    await expect(writeAccessLinkLockdownDenylist({ DENYLIST: { put } }, "art_1")).resolves.toBe(true);
    expect(put).toHaveBeenCalledWith("ad:art_1", expect.stringContaining('"reason":"access_link_lockdown"'), {
      expirationTtl: resolveUsagePolicy({ billingEnabled: false }).max_ttl_seconds,
    });
  });
  ...
  it("deletes ad: keys when access-link lockdown is lifted", async () => {
    ...
    await expect(deleteAccessLinkLockdownDenylist({ DENYLIST: { put: vi.fn(), delete: del } }, "art_1")).resolves.toBe(
      true,
    );
  ...
  const activeArtifact = (overrides: Partial<{ access_link_lockdown_at: string | null }> = {}) => ({
    id: "art_1",
  ...
  it("retains ad: on platform-lockdown lift when an access-link lockdown remains", async () => {
    const repo = new LocalRepository({ apiKeyPepper: "pepper" });
    const artifact = activeArtifact({ access_link_lockdown_at: "2026-01-02T00:00:00.000Z" });
    repo.artifacts.set(artifact.id, artifact);
    await expect(repo.peekArtifactPlatformLockdownRetention("art_1")).resolves.toBe(true);
  });

  it("releases ad: on platform-lockdown lift when no access-link lockdown remains", async () => {
    const repo = new LocalRepository({ apiKeyPepper: "pepper" });
    const artifact = activeArtifact();
    repo.artifacts.set(artifact.id, artifact);
    await expect(repo.peekArtifactPlatformLockdownRetention("art_1")).resolves.toBe(false);
  });

  it("retains ad: defensively when the artifact is missing", async () => {
    const repo = new LocalRepository({ apiKeyPepper: "pepper" });
