...
export const platformLockdownQueries = {
  ...
  async insert(db: DrizzleDb, row: PlatformLockdown) {
    await db.insert(platformLockdowns).values({
      id: row.id,
      scope: row.scope,
  ...
  async markLifted(db: DrizzleDb, id: string, input: { liftedAt: string; liftedBy: string }) {
    await db
