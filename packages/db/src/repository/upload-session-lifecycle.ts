...
import { DEFAULT_UPLOAD_SESSION_TTL_MS, type UsagePolicyConfig } from "../policy.js";
...
export async function createUploadSessionInEntities(
  ...
  const session: UploadSession = {
    ...
    artifact_expires_at: new Date(
      new Date(input.now).getTime() + (input.request.ttl_seconds ?? input.usagePolicy.default_ttl_seconds) * 1000,
    ).toISOString(),
