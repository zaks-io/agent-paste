...
export function postgresArtifacts(ctx: PostgresContext): Entities["artifacts"] {
  ...
  return {
    ...
    markDeleted: (artifactId, deletedAt) =>
      withSource("postgresArtifacts.markDeleted", async () => {
        await sql.query(
          `update artifacts
    ...
    listExpiring: (now, limit) =>
      withSource("postgresArtifacts.listExpiring", async () => {
    ...
    expireBatch: (now, ids) =>
      withSource("postgresArtifacts.expireBatch", async () => {
        await sql.query(
          `update artifacts
...
function withSource<T>(functionName: string, run: () => T): T {
  return withSqlQuerySource(
    {
      filepath: "packages/db/src/repository/postgres-entities/artifacts.ts",
      functionName,
      namespace: "packages.db.src.repository.postgres-entities.artifacts",
    },
    run,
  );
}
