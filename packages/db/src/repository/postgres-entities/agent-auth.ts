...
export function postgresAgentAuth(ctx: PostgresContext): Entities["agentAuth"] {
  ...
  return {
    ...
    findDelegationByIdForUpdate: (id) => agentAuthQueries.findDelegationByIdForUpdate(drizzle, id),
    ...
    findRegistrationByIdForUpdate: (id) => agentAuthQueries.findRegistrationByIdForUpdate(drizzle, id),
    findRegistrationByClaimTokenHash: (hash) => agentAuthQueries.findRegistrationByClaimTokenHash(drizzle, hash),
    checkVerifiedClaimAttempt: (input) => agentAuthQueries.checkVerifiedClaimAttempt(drizzle, input),
