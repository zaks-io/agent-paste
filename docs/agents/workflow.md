...
2. Implementation
   ...
   Use `agent-paste-implement-issue` for one Linear issue and one branch. The
...
4. PR handoff
   ...
   Use `agent-paste-create-pr` after the local review is clean. PRs should be
   ready for review, include the Linear issue, summarize checks, and move the
   issue to `In Review`.

5. PR review and fix loop

   Use `agent-paste-review-pr` to review the PR against the Linear issue,
