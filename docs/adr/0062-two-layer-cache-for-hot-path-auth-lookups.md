...
Status note: amended by AP-294. The L2 synthetic cache URL shape below is historical; the shipped helper uses a different internal origin and path. Current behavior is in [`docs/specs/architecture.md`](../specs/architecture.md#auth-lookup-cache).
...
- **L2 — `caches.default`** keyed by a synthetic `Request` URL of shape `https://cache.agent-paste.internal/{namespace}/{key}` with `Cache-Control: max-age={ttl}` on the stored `Response`. The synthetic origin is unreachable from outside; only the colocated Worker reads or writes it. **Amended (AP-294):** the shipped helper in `packages/auth` keys L2 with `https://agent-paste.internal/cache/{namespace}/{key}` (namespace and key are URI-encoded). See [`docs/specs/architecture.md`](../specs/architecture.md#auth-lookup-cache).
