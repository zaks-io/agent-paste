...
- `content` validates: parse the URL, confirm `kid` is current or within the overlap window, check `exp > now()`, recompute the HMAC, constant-time compare. Any failure returns the generic `not_found` per ADR 0036.
