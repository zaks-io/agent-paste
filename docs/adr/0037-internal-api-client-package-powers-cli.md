...
- Constructor: `new ApiClient({ auth?, apiBaseUrl?, uploadBaseUrl?, contentBaseUrl?, apiVersion?, uploadConcurrency? })`. `auth` resolves either an **API Key** (`{ type: 'api_key', apiKey }`) or a WorkOS bearer provider (`{ type: 'bearer', getAccessToken }`) for the one-shot CLI key-mint path from [ADR 0060](./0060-cli-authentication-via-auth0-loopback.md). When omitted, the client checks `AGENT_PASTE_API_KEY` before falling back to the CLI credential store. Base URLs default to the production hosts established by ADR 0014. `apiVersion` defaults to `v1` and is configurable for forward-compat probing per ADR 0023. `uploadConcurrency` defaults to 4.
- The same resolved control-plane credential authenticates to `api` and `upload`; `content` reads use short-lived content-gateway URLs returned by the API. The SDK does not maintain separate user credentials per host.
