...
- User-facing **Publish Result** surfaces should return `access_link_url`, the **Access Link Signed URL** minted from a **Share Link**, for the stable live viewer. The authenticated **Artifact URL** is for workspace management, and `agent_view_url` is for the publishing actor, not for unauthenticated recipients. Recipients derive their **Agent View** through the resolve endpoint above when they receive an **Access Link Signed URL**.
...
- **Publisher picks the URL by audience**: expect the publishing agent to send a **Revision Link** to humans and an authenticated **Agent View** URL to agents. Rejected: the publisher rarely knows whether the recipient is human, agent, or both, and authenticated **Agent View** URLs are not durable unauthenticated handoff links. The default human handoff is now the **Access Link Signed URL** minted from a **Share Link**; explicit **Revision Links** are only for pinned-version handoff.
