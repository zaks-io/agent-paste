...
- The apex `agent-paste.sh` serves only marketing/docs and a 308 redirect to `app.agent-paste.sh` for any path that resolves to product surfaces. The apex never hosts authenticated state, never receives WorkOS callbacks, and never sets cookies.
