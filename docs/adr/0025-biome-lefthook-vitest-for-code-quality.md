...
- Knip configuration lives in `knip.json` at the repo root. The root `pnpm knip` command supplies a placeholder `DATABASE_URL` only when one is missing so Knip can load the existing Drizzle config without requiring live database credentials.
- Lefthook configuration lives in `lefthook.yml` at the repo root. Hooks are heavy: pre-commit runs staged Biome, Prettier on Markdown, `gitleaks protect --staged --redact`, and affected Turbo typecheck with an `origin/main` base that falls back to `HEAD`; pre-push runs `pnpm knip` and `pnpm test:coverage` so dead-code and global coverage checks run before local pushes.
