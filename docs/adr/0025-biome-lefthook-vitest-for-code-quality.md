...
- Lefthook configuration lives in `lefthook.yml` at the repo root. Hooks are heavy: pre-commit runs staged Biome, Prettier on Markdown, `gitleaks protect --staged --redact`, and affected Turbo typecheck with an `origin/main` base that falls back to `HEAD`; pre-push runs `pnpm test:coverage` so global coverage thresholds match CI.
