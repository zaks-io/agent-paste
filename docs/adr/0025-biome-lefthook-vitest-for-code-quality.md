...
- Lefthook configuration lives in `lefthook.yml` at the repo root. Hooks are heavy: pre-commit runs `biome check --write --staged`, `gitleaks protect --staged`, and `turbo run typecheck --filter=...[origin/main]`; pre-push runs `pnpm test:coverage` so global coverage thresholds match CI.
