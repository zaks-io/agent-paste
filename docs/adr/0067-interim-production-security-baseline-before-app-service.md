...
- Legacy unsigned Agent View tokens shaped `{artifactId}.{revisionId}` are disabled by default. They are allowed only when an explicit local/dev escape hatch is set.
