...
- [ADR 0060](./0060-cli-authentication-via-auth0-loopback.md) is the canonical interactive CLI auth model: `agent-paste login` via WorkOS loopback PKCE (decided off Auth0, consistent with ADR 0068). `agent-paste login` is primary for humans; `AGENT_PASTE_API_KEY` remains for CI/headless use.
...
- [ADR 0068](./0068-workos-authkit-for-web-app-auth.md) supersedes [ADR 0002](./0002-auth0-for-workspace-authentication.md) for `apps/web`. The dashboard authenticates through WorkOS AuthKit via the official `@workos/authkit-tanstack-react-start` integration. The CLI provider is now decided as WorkOS loopback PKCE ([ADR 0060](./0060-cli-authentication-via-auth0-loopback.md)); MCP ([ADR 0061](./0061-mcp-worker-with-oauth-only-via-auth0-dcr.md)) is expected to follow WorkOS but is formally re-decided when that surface is built.
