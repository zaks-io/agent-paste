...
- **Migrations run as `platform_admin`** from GitHub Actions per [ADR 0007](./0007-database-migrations-and-preview-environments.md). The DB connection string used by CI is a separate credential from the one Hyperdrive uses to reach `app_role`; CI does not need the Hyperdrive route. **Amended by [ADR 0077](./0077-migrations-run-as-neondb-owner-not-platform-admin.md):** in the deployed system migrations actually run as the Neon owner role (`neondb_owner`), not `platform_admin`, because `platform_admin` is not the owner of any table and cannot run ownership-only DDL. The first migration still _creates_ both roles; only the migration _runner_ differs.
