...
The **Agent View** response wraps the **Manifest** from [ADR 0053](./0053-manifest-shape-and-creator-visibility.md) with five sibling fields: `display_metadata`, `files`, `content_prefix`, `safety_warnings`, and `bundle`. Files reference paths under a single signed `content_prefix` from [ADR 0028](./0028-signed-url-tokens-for-content-gateway-authorization.md) — one token per resolved **Revision**, not a per-file URL map. **Safety Warnings** carry a stable snake_case `code` (the contract), a `severity` of `info` or `warning`, a `scope` of `artifact`, `revision`, or `file`, an optional `file_path`, a sanitized plain-text `message`, and `detected_at`. **Bundle Availability** is a single object with `status`, an optional `url` when ready, `bytes`, `error_code`, and `generated_at`.
...
- **Bundle status string only.** Matches the CLI README's first cut. Forces a follow-up call to retrieve the **Bundle** URL or `error_code`. Rejected because the envelope is already an aggregate; returning the URL inline when ready is one fewer round trip and matches the prefix-on-resolve pattern.
...
- **Bundle** `url` is short-lived and shaped `usercontent.agent-paste.sh/b/{token}` per [ADR 0028](./0028-signed-url-tokens-for-content-gateway-authorization.md). It is omitted (`null`) until **Bundle Availability** transitions to `ready`.
