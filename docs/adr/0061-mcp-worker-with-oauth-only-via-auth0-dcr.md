...
- **Publish chain.** As with the CLI per [ADR 0037](./0037-internal-api-client-package-powers-cli.md), one user-visible publish key threads through `upload.session.create`, `upload.session.finalize`, `api.publish`, required `access_link.create` for the **Revision Link**, and optional `access_link.create` when `share` is requested. Records are distinguished by operation and, for access-link creates within the same publish, by derived idempotency-key suffixes (`:revision-link`, `:share-link`) so revision and share minting do not collide on retry.
