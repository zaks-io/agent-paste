...
> **Superseded in part by [ADR 0079](./0079-mcp-scopes-derived-from-member-role-not-workos-token.md), [ADR 0084](./0084-cli-and-mcp-share-one-publish-path.md), [ADR 0086](./0086-publish-is-content-only-private-first.md), [ADR 0091](./0091-client-side-revise-engine-and-literal-edit-tools.md), and [ADR 0094](./0094-capability-url-is-the-artifact-link.md).** This ADR assumes the consent screen requests `{write, read, share}` as OAuth scopes and that WorkOS mints them into the token's `scope` claim. WorkOS AuthKit does not issue custom OAuth scopes: `scopes_supported` is fixed to `{email, offline_access, openid, profile}`, and Permissions/Roles are a separate system that never become requestable `scope` values. A caller's granted MCP scopes are therefore derived from their **Workspace Member** inside `api`, not read from the token. The original twelve-tool, `write`/`read`/`share`, `share` flag, and Share Link details below are historical. The shipped MCP surface is the ten-tool `read`/`publish` registry in [`packages/contracts/src/mcp/registry.ts`](../../packages/contracts/src/mcp/registry.ts); publish returns the Artifact's capability URL, and targeted edits use `multi_edit`. The transport, OAuth-via-AuthKit, CIMD/DCR registration, and resource-indicator `aud` binding still stand.
...
| ------------------------------------------------------------------------ | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `publish_artifact(title, body, render_mode, share?, idempotency_key?)`   | `write read`   | New **Artifact**, single file. `share` defaults to `false`; set `share: true` only when the user explicitly asks for a shareable no-login **Share Link**, returning its **Access Link Signed URL** as `access_link_url`.                      |
| `add_revision(artifact_id, body, render_mode, share?, idempotency_key?)` | `write read`   | New **Revision** on existing **Artifact**. `share` defaults to `false`; set `share: true` only when the user explicitly asks for a shareable no-login **Share Link**, reusing an active Share Link when possible or creating one when needed. |
