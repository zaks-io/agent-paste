...
> **Superseded in part by [ADR 0079](./0079-mcp-scopes-derived-from-member-role-not-workos-token.md), [ADR 0084](./0084-cli-and-mcp-share-one-publish-path.md), [ADR 0086](./0086-publish-is-content-only-private-first.md), and [ADR 0091](./0091-client-side-revise-engine-and-literal-edit-tools.md).** This ADR assumes the consent screen requests `{write, read, share}` as OAuth scopes and that WorkOS mints them into the token's `scope` claim. WorkOS AuthKit does not issue custom OAuth scopes — `scopes_supported` is fixed to `{email, offline_access, openid, profile}`, and Permissions/Roles are a separate system that never become requestable `scope` values. A caller's granted MCP scopes are therefore derived from their **Workspace Member** inside `api`, not read from the token. The original twelve-tool, `write`/`read`/`share`, `share` flag, and `create_share_link` details below are historical. The shipped MCP surface is the fourteen-tool `read`/`publish` registry in [`packages/contracts/src/mcp/registry.ts`](../../packages/contracts/src/mcp/registry.ts); publish is content-only/private, unlisted sharing is `set_visibility`, and targeted edits use `multi_edit`. The transport, OAuth-via-AuthKit, CIMD/DCR registration, and resource-indicator `aud` binding still stand.
