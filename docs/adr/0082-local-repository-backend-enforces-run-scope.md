...
- **Workspace Run Scope** (`{ kind: "workspace", workspaceId: W }`) — the view exposes only rows whose `workspace_id === W`, with this split:
  ...
  - **Foreign write throws** (loud, self-labeling). A write that carries an explicit `workspace_id` — an `insert`, or a mutation that takes a workspace argument such as `updateTitle(artifactId, workspaceId, …)` or `markRetained({ …, workspaceId })` — throws when that id does not match the **Run Scope**. Writing a row into the wrong tenant is never legitimate, so it surfaces as a failing test with a cross-tenant message rather than a silent no-op.
  - **Id-only mutation no-ops** (RLS-faithful). A mutation that targets a row by id alone (`markDeleted`, `revoke`, `setPinnedAt`) finds nothing in the scoped view for a foreign row, so it affects zero rows — exactly as an RLS-scoped `UPDATE` would.
...
- **Enforce at the unit-of-work seam via a Scoped View; silent-empty on reads, throw on workspace-bearing writes (chosen).** One enforcement point, no per-method knowledge of which argument is the target id, RLS-faithful where RLS is silent, loud where a write provably crosses the boundary.
