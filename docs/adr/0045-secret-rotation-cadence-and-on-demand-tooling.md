...
- **Signing keys (ADR 0028, 0031, 0047): 90-day automatic rotation.**
...
- **API Key pepper (ADR 0043): on-demand only with continuously tested tooling.**
  ...
  - The rotation tooling is reached through the operator-only admin surface on `api` per ADR 0046 (`POST /admin/rotations/api-key-pepper`); the CLI in `apps/cli` does not expose it.
  ...
  - The transient-default artifact lifecycle (ADR 0032) bounds the realistic exposure window of any stored credential, so the gap between scheduled and on-demand cadence is small in practice.
...
- **Auth0 session secrets** are managed by Auth0 and are not in scope here. Pepper rotation is independent of Auth0 rotation.
