...
- **The Worker keeps edge pre-flight gating for multi-step tools.** Before a tool that makes multiple authenticated calls, the Worker fetches the member's derived scope set from `api` (via `mcp.whoami`) and checks the tool's `requiredScopes`, returning `insufficient_scope` before the chain can fail partway through. A single-call tool relies on that one `api` request's authentication and scope guard, so it does not resolve the same member twice. `api` maps the same missing-scope result back to `insufficient_scope`.
- **`api` remains the authority.** Edge gating is an optimization, not the security boundary. Under [ADR 0097](./0097-mcp-private-principal-handoff.md), `api` resolves the verified subject received over its private named RPC entrypoint and enforces the current member role/RLS on every forwarded call. Public HTTP callers cannot supply that internal principal.
...
- **`apps/mcp`**: `verifyMcpOAuthToken` verifies issuer + `aud` and returns `{ tokenSub }` only — no `scopes`. Multi-step tools resolve the member scope set with `mcp.whoami` before starting; single-call tools reuse the authenticated principal and scope guard of their one `api` request. `parseMcpScopeClaim` and `mcpScopeClaimIncludesMemberOnlyScopes` usage is removed from the verification path.
