...
- **The resource indicator drives `aud`.** Configuring `https://mcp.agent-paste.sh` as a Resource Indicator makes AuthKit mint tokens with `aud=https://mcp.agent-paste.sh`. Without it, AuthKit ignores the `resource` parameter and stamps a default environment audience. This part of ADR 0061 is correct and is retained.
...
- **WorkOS configuration is complete and frozen for MCP**: DCR + CIMD enabled, resource indicator `https://mcp.agent-paste.sh` set per environment. No Permissions, Roles, or custom-scope configuration in WorkOS is required for MCP. The earlier per-environment Permission/Role setup done while diagnosing this is inert for MCP and may be removed.
