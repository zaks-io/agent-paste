...
- Raw bytes only (no renderer): simplest worker, but markdown and directory views show as `text/plain` or 404, which is unacceptable UX for two of the seven MVP **Render Modes**.
...
- Directory **Render Mode** is rendered as a file listing whose entries are signed-prefix-relative links to children of the same **Revision**; no `api` call is required to enumerate files.
