...
- `jobs` should consume queue messages for artifact byte purge, safety scanning, retention cleanup, upload cleanup, and generated bundle maintenance.
...
- Routine upload cleanup that only removes unpublished bytes should emit operational logs, not audit events.
- Upload cleanup that removes stale unpublished artifact management state should create audit events with a system actor.
- Bundle generation failures should emit operational logs and alerts when appropriate, not audit events, unless they change product-visible state beyond bundle status.
