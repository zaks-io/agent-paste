...
- `apps/api` verifies WorkOS access tokens via JWKS at `https://api.workos.com/sso/jwks/<client_id>` and matches the `client_id` claim. The audience-based check sketched in ADR 0059 is replaced; the rest of the resolution flow (Workspace Member upsert and RLS scope) is unchanged.
- Route loaders call `getAuth()` directly from `@workos/authkit-tanstack-react-start` and compose `is_operator` at the call site from the WorkOS `admin` role slug (`role` / `roles` claims). No wrapper layer; WorkOS SDK types are the canonical user shape in components. The canonical route paths are `/api/auth/sign-in` (GET, 307 to WorkOS), `/api/auth/callback` (GET, `handleCallbackRoute`), and `/api/auth/sign-out` (POST, `signOut()`).
- Environment variables: `WORKOS_CLIENT_ID`, `WORKOS_API_KEY`, `WORKOS_REDIRECT_URI`, `WORKOS_COOKIE_PASSWORD` (32+ chars), `WORKOS_COOKIE_NAME=__agp_session`, and `WEB_BASE_URL`. The Auth0 vars (`AUTH0_DOMAIN`, `AUTH0_CLIENT_ID`, `AUTH0_CLIENT_SECRET`, `AUTH0_AUDIENCE`, `WEB_SESSION_SEAL_KEY_V1`) are removed; no backwards-compat shim because nothing was deployed.
...
- The operator boundary ([ADR 0046](./0046-operator-identity-and-web-admin-surface.md)). `is_operator` is derived from the WorkOS `admin` role slug in the active session.
...
- Extend `scripts/bootstrap-secrets.mjs` to push `WORKOS_API_KEY`, `WORKOS_COOKIE_PASSWORD`, and `WORKOS_CLIENT_ID` to both Workers.
