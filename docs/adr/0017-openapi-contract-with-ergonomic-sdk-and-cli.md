...
- Superseded in part by ADR 0037: the CLI's HTTP layer now lives in the workspace-only `packages/api-client/` package, and the CLI surface extends beyond `publish` to include management verbs (list, get, delete, access-link CRUD, lockdown, download, whoami). The package remains unpublished, so the REST API stays the canonical public integration surface.
- The shared-contracts package gestured at above is realized by ADR 0038 as `packages/contracts/`, with Zod schemas as the source of truth for both backend validation and SDK type inference.
