...
- Superseded in part by [ADR 0060](./0060-cli-authentication-via-auth0-loopback.md): the primary interactive CLI auth path is `agent-paste login` via WorkOS loopback. `AGENT_PASTE_API_KEY` remains the CI/headless path, and the CLI still does not accept secrets as flags.
