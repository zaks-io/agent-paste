...
Status: Accepted. Implemented per [ADR 0066](./0066-cli-first-mvp-contract-narrowing.md) Phase 6 hardening.
...
CLI-first MVP contracts and route implementations now carry the encryption metadata, per-object `kid` handling, and Worker encryption/decryption paths required by this ADR while retaining private R2, signed content tokens, isolated `usercontent.agent-paste.sh` serving, and no direct R2 URLs.
...
- **`upload` encrypts before R2 PUT.** Direct-to-R2 signed PUT URLs from [ADR 0027](./0027-upload-write-path.md)'s original shape cannot satisfy this ADR, so the signed URL now points to an `upload` Worker route. The agent PUTs plaintext bytes to `upload`; `upload` validates the URL, expected path, and `Content-Length`, then encrypts before writing the reserved final key to R2. The signed-URL pattern is retained for the per-**Upload Session** capacity allocation and reserved-key semantics; the bytes do not go straight from agent to R2.
