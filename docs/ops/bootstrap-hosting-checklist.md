...
| `LINEAR_ACCESS_KEY`                    | GitHub Actions (`deploy-production.yml` release step)          | Linear release pipeline access key scoped to the Agent Paste team. Store as a repository secret; successful production deploys record the deployed commit SHA as the Linear release version after smoke passes.                                             |
