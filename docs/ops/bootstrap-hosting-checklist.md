...
| `TURBO_TOKEN`                          | Trusted CI / deploy workflows (remote cache)                   | `zaks-io` org secret when inherited, or repo secret if org inheritance is unavailable. Public external PR CI is intentionally disabled until a no-secret path exists.                                                                                       |
| `TURBO_TEAM`                           | Trusted CI / deploy workflows (remote cache)                   | `zaks-io` org var (`zaks-io`) when inherited, or repo variable if org inheritance is unavailable. Public external PR CI is intentionally disabled until a no-secret path exists.                                                                            |
