...
| `TURBO_TOKEN`                          | Optional remote cache for trusted CI / deploy workflows        | `zaks-io` org secret when inherited, or repo secret if org inheritance is unavailable. PR validation falls back to local cache when absent; GitHub withholds it from public fork PRs.                                                                                                                                  |
| `TURBO_TEAM`                           | Optional remote cache for trusted CI / deploy workflows        | `zaks-io` org var (`zaks-io`) when inherited, or repo variable if org inheritance is unavailable. Public fork PRs may receive this non-secret identifier, but without `TURBO_TOKEN` they still use local cache.                                                                                                        |
| `TURBO_REMOTE_CACHE_SIGNATURE_KEY`     | Trusted CI / deploy workflows and local development            | Generate once: `openssl rand -hex 32`. Set as repo or org secret. GitHub withholds it from public fork PRs, which use local Turbo caching.                                                                                                                                                                             |
...
- [ ] `agent-paste / infra / DATABASE_URL_MIGRATIONS_PRODUCTION` (production-branch direct URL, NOT pooled, role `neondb_owner`). **Isaac only** to copy from Neon and store in Bitwarden unless a CLI/API credential is already available; **Codex can handle** GitHub storage after the value is available in the shell.
