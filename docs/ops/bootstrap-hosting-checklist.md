...
- `publish` returns `artifact_id` (`art_*`), `revision_id` (`rev_*`), a `private_url` rooted at `https://app.agent-paste.sh` (`/v/<artifactId>`), a `revision_content_url` on `https://{capability-id}-uc.agent-paste.sh`, and an `agent_view_url` rooted at `https://api.agent-paste.sh`.
