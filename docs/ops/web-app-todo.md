...
- [x] Lighthouse a11y check on `/dashboard` (empty state). `scripts/lighthouse-dashboard-a11y.mjs` runs locally against mock WorkOS + the built web Worker; PR preview fails below 95 via `pnpm lighthouse:dashboard-a11y` after hosted smoke (runs even when per-PR web deploy is skipped).
