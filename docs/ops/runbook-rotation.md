...
Current status: WorkOS AuthKit is the current web auth stack. Preview and production WorkOS login work. The completed Phase 3 web close-out is recorded in [`status/phase-backlog.md`](./status/phase-backlog.md#active-phase-3-close-out) and [`web-app-todo.md`](./web-app-todo.md); current work lives in the issue tracker.
