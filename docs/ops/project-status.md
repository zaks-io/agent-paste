...
Last updated: 2026-05-23 (repository adapters unified behind a backend-agnostic `RepositoryCore` and a single exported `Repository` interface per ADR 0069; first dashboard read API tranche implemented after WorkOS callback; ADR 0057 denylist key drift reconciled for the CLI-first MVP; ADR 0045 rotation groundwork runbook added; web auth swapped from Auth0 to WorkOS AuthKit per ADR 0068; workspace member DB foundation and web Worker secret bootstrap follow-ups completed; Logpush and production deploy-gate work parked for later).
...
| --------------------- | ---------------------- | ---------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/api`            | Implemented            | ~950       | Yes   | `src/index.ts`. Hono routing, `/openapi.json`, public Agent View, admin routes, scheduled cleanup, signed content URLs, denylist writes, WorkOS web callback member provisioning, and first dashboard read APIs (`/v1/web/workspace`, `/v1/web/artifacts`, `/v1/web/artifacts/{artifactId}`).                                                                                                                                                                                                                                                                                                                                                            |
...
| `docs/specs/web.md`               | Partial | TanStack Start scaffold implements every route from the spec with WorkOS AuthKit + sealed sessions per ADR 0068. `apps/api` now serves the workspace and artifact dashboard read endpoints; remaining loaders still render `EmptyState` until their `/v1/web/*` endpoints land -- see [`web-app-todo.md`](./web-app-todo.md). |

## ADR Coverage

All 69 ADRs in numeric order. Audit on 2026-05-23 confirmed every `docs/adr/0001` through `0069` file has a row here; rows 0029/0030 were corrected to match the ADR filenames. Status legend: **Done**, **Partial**, **Drift** (code differs from ADR by intent), **Deferred** (post-MVP per ADR 0066 or phases.md), **Superseded**.
...
| Phase 3 — Public OAuth + web  | WorkOS AuthKit, signup, dashboard, Access Links           | ~20%   | TanStack Start scaffold + style-guide tokens + WorkOS AuthKit wiring shipped per ADR 0068; `workspace_members` schema is in place; first dashboard read APIs for workspace context and artifacts are implemented. Remaining: WorkOS project click-ops, keys/audit/settings/admin `/v1/web/*` endpoints, real loader wiring. See [`web-app-todo.md`](./web-app-todo.md). |
...
- Drives: ADR 0069 (new); ADR 0022, ADR 0035, ADR 0044 (preserved).
- Files: `packages/db/src/repository/{core,interface,ports,shared,web-transforms,postgres-entities,postgres-unit-of-work,local-entities,local-state,local-unit-of-work}.ts` (new), `packages/db/src/{index,index.test,local-repository}.ts`, `packages/db/src/postgres/repository.ts`, `apps/api/src/index.ts`, `apps/upload/src/index.ts`, `apps/api/src/local-mvp.test.ts`, `apps/upload/src/{index,error-envelope}.test.ts`, `docs/adr/0069-repository-core-ports-and-adapters.md`, `docs/adr/README.md`, `docs/ops/repository-todo.md`.
