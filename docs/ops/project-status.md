...
- `origin/main` is at `ba78ff5 Add dashboard workspace artifact reads (#34)`, atop the WorkOS callback and web foundation work from 2026-05-23. This branch adds the next content hardening and dashboard API tranche.
- Latest feature commits on `main`: `ba78ff5` (#34 dashboard workspace/artifact reads), WorkOS callback/member schema/bootstrap follow-ups, and ADR 0057 denylist reconciliation. Earlier runtime commits are bug fixes (#20 migration idempotency, #21 Bug A); #22 is status docs.
...
| 0033 TanStack Start web app               | Partial      | Scaffold landed: TanStack Start on Cloudflare Workers via `@cloudflare/vite-plugin`, file routes, sealed `__agp_session` cookie, WorkOS AuthKit via `@workos/authkit-tanstack-react-start` (ADR 0068), service binding to `api`. Workspace and artifact dashboard read endpoints exist; remaining `/v1/web/*` loaders fall back to `EmptyState` until their endpoints land. Tracked in [`web-app-todo.md`](../ops/web-app-todo.md).                         |
