...
Last updated: 2026-05-24 (ADR 0071 signed-token codec / `packages/tokens` is implemented per [`signed-tokens-todo.md`](./signed-tokens-todo.md); ADR 0072 contract-driven route registrar / `packages/worker-runtime` is accepted and pending implementation per [`worker-runtime-todo.md`](./worker-runtime-todo.md). Prior: artifact-level content read throttling; dashboard API key lifecycle and cursor-paginated audit reads; repository adapters unified behind a backend-agnostic `RepositoryCore` per ADR 0070; Logpush and production deploy-gate work parked for later).
...
| 0072 contract-driven route registrar      | Accepted (pending) | New 2026-05-24. `@agent-paste/worker-runtime` not yet created; routes still mount via per-Worker inlined guard chains and `withWebMember`. Adds `rateLimit` to the Route Contract. Implementation in [`worker-runtime-todo.md`](./worker-runtime-todo.md).                                                                                                                                                                                                  |
...
### 2. Make the route contract the runtime enforcement source via `packages/worker-runtime`

- Drives: ADR 0072; ADR 0034/0035/0036/0039/0064 (preserved).
- Files: `packages/worker-runtime/*` (new), `packages/contracts/src/routes.ts` (add `rateLimit`), `apps/api/src/index.ts`, `apps/upload/src/index.ts`, `apps/content/src/index.ts`. Full ordered steps in [`worker-runtime-todo.md`](./worker-runtime-todo.md).
- Done: every route in the three Workers mounts through `createRegistrar(deps).mount(contract, handler)`; the inlined guard chains and `withWebMember` are deleted; the `ERROR_STATUS` table is reconciled against current statuses; `pnpm verify` green and hosted smoke unchanged.
