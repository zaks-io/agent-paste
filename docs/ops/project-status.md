...
Last updated: 2026-05-23 (artifact-level content read throttling implemented; dashboard API key lifecycle and cursor-paginated audit reads added after the first dashboard read API tranche; Logpush and production deploy-gate work remain parked for later).
...
- `origin/main` is at `2695bf0 docs: record Live Updates design as ADR 0069 (#37)`, atop the dashboard workspace/artifact read work from 2026-05-23. This branch adds the next content hardening and dashboard API tranche.
- Latest feature commits on `main`: `2695bf0` (#37 live updates design), `ba78ff5` (#34 dashboard workspace/artifact reads), WorkOS callback/member schema/bootstrap follow-ups, and ADR 0057 denylist reconciliation. Earlier runtime commits are bug fixes (#20 migration idempotency, #21 Bug A); #22 is status docs.
...
- `apps/web` is a full TanStack Start scaffold (WorkOS AuthKit, twelve routes, `EmptyState` loaders) per ADR 0033/0068. `api` now has the workspace/artifact read, key lifecycle, and cursor-paginated audit read dashboard endpoints. `jobs` and `mcp` remain Hono scaffolds only: `healthz` + `/openapi.json` + no business logic.
...
- Unauthenticated content reads call a native Cloudflare rate-limit binding per Artifact and return the `rate_limited_artifact` envelope with `Retry-After`.
...
| --------------------- | ---------------------- | ---------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/api`            | Implemented            | ~950       | Yes   | `src/index.ts`. Hono routing, `/openapi.json`, public Agent View, admin routes, scheduled cleanup, signed content URLs, denylist writes, WorkOS web callback member provisioning, dashboard workspace/artifact reads, dashboard API key lifecycle, and cursor-paginated dashboard audit reads.                                                                                                                                                                                                                                                                                                                                                           |
...
| `docs/specs/mvp.md`               | Partial | Publish/read flows work; delete/expiry byte-purge is verified in local/hosted smoke. Remaining MVP-shaped hardening is interim operator identity follow-ups. Rotation has an MVP runbook; automation remains future work.                                                                                                                  |
...
| `docs/specs/features.md`          | Partial | MVP CLI + 3 Workers implemented, including artifact-level content read throttling. Future features (multi-revision, OAuth, MCP, dashboard completion) remain phased.                                                                                                                                                                       |
...
| 0024 treat agent data as untrusted        | Partial      | CSP + private R2 + signed URLs in place. 2026-05-21 pass fixed upload MIME trust and content headers; artifact-level content read throttling now protects signed reads.                                                                                                                                                                                                                                                                                             |
...
| 0033 TanStack Start web app               | Partial      | Scaffold landed: TanStack Start on Cloudflare Workers via `@cloudflare/vite-plugin`, file routes, sealed `__agp_session` cookie, WorkOS AuthKit via `@workos/authkit-tanstack-react-start` (ADR 0068), service binding to `api`. Workspace/artifact reads, API key lifecycle, and cursor-paginated audit dashboard endpoints exist; remaining loaders fall back to `EmptyState` until their endpoints land. Tracked in [`web-app-todo.md`](../ops/web-app-todo.md). |
...
| ----------------------------- | --------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phase 1 — CLI-first MVP       | Real hosted publish loop, expiration, observability floor | ~96%   | Final hosted smoke after content read-throttle changes.                                                                                                                                                                                                                                                                                                                                        |
...
| Phase 3 — Public OAuth + web  | WorkOS AuthKit, signup, dashboard, Access Links           | ~25%   | TanStack Start scaffold + style-guide tokens + WorkOS AuthKit wiring shipped per ADR 0068; `workspace_members` schema is in place; workspace/artifact reads, API key lifecycle, and cursor-paginated audit reads are implemented. Remaining: WorkOS project click-ops, settings/admin `/v1/web/*` endpoints, Access Links, and real loader wiring. See [`web-app-todo.md`](./web-app-todo.md). |
...
- Files: `apps/api/src/index.ts`, `apps/api/src/index.test.ts`, `apps/cli/src/index.ts`, `packages/commands/src/index.ts`, `packages/contracts/src/{apiKeys,enums,routes}.ts`, `packages/contracts/src/openapi/api.ts`, `packages/contracts/openapi/api.json`, `packages/db/migrations/0005_member_actor_events.sql`, `packages/db/snapshot/schema.sql`, `packages/db/src/{index.test,local-repository,types,schema}.ts`, `packages/db/src/postgres/{repository,rls.test}.ts`, `packages/db/src/queries/operation-events.ts`, `docs/ops/web-app-todo.md`.
