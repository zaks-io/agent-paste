...
| `docs/specs/mvp.md`               | Partial | Publish/read flows work; delete/expiry byte-purge is verified in local/hosted smoke. Remaining MVP-shaped hardening is interim operator identity and tested rotation automation follow-ups.                                                                    |
...
| `docs/specs/features.md`          | Partial | MVP CLI + 3 Workers implemented. Future features (multi-revision, OAuth, MCP, dashboard) are absent by design.                                                                                                                                                 |
...
| `docs/specs/acceptance.md`        | Partial | Local/hosted smoke now cover publish/read/delete/expiry byte-purge, Agent View, operation events, and rate-limit 429s. Remaining gap is broader preview/load coverage.                                                                                         |
...
| 0024 treat agent data as untrusted        | Partial      | CSP + private R2 + signed URLs in place. 2026-05-21 pass fixed upload MIME trust and content headers; content artifact read throttling is implemented.                                                                                                                                                                                                                                |
...
| 0048 transient artifacts by default       | Partial      | TTL defaults, delete/expiry stop-serving behavior, R2 byte purge, denylist writes, and artifact-level unauthenticated read throttling are verified. Pinning remains deferred.                                                                                                                                                                                                         |
...
### 1. Run final hosted smoke after denylist/throttle deploy

- Drives: ADR 0028, ADR 0048, ADR 0057, `docs/specs/acceptance.md`.
- Files: hosted smoke only; code should already be merged from the denylist and content throttling PRs.
- Done: preview and production smoke confirm publish/read/delete/expiry, `ad:` denylist key behavior, and the `rate_limited_artifact` envelope on hosted Workers after #29/#31 land.
...
- Done: `content` declares an optional Cloudflare native `ARTIFACT_RATE_LIMIT` binding at 60 req/min and keys it by `artifact_id` from the resolved content-token payload rather than the raw token or URL. The Worker verifies token signature/path first, checks KV denylist keys next, verifies the R2 object exists, and only then consumes the artifact rate-limit bucket, so deleted/revoked/missing content still returns the stable generic 404 and throttling cannot bypass denylist. Over-limit reads return the public `rate_limited_artifact` 429 envelope with `Retry-After: 60`; absent or failing bindings fail open per the existing app convention.
