...
Last updated: 2026-05-21 (idempotent mutations landed).
...
- `main` is at `7354630 docs: replace MVP checklist with project status doc`.
...
| `apps/cli` | Implemented | ~520 | Yes | `src/index.ts`, `src/local.ts`. `whoami`, `publish`, admin commands. Missing destructive `--yes` guards. |
...
| `packages/commands` | Implemented | ~150 | Yes | `runCommand` + `createOperationEvent` + idempotency helpers. Wired into every mutation route in `api` and `upload`; replay is durable via `idempotency_records`. |
...
| `docs/specs/mvp.md` | Partial | Publish/read flows work; cleanup byte-purge and admin `--yes` guards unverified. |
...
| `docs/specs/data-model.md` | Partial | Schema + migration match. RLS roles defined in migration but Workers don't `SET LOCAL app.workspace_id`. Operation events present; full audit pattern (ADR 0004) not wired. |
| `docs/specs/content-rendering.md` | Partial | Content Worker handles signed tokens, extensions, denylist, CSP. SVG strict-CSP and CDN allowlist need explicit validation. |
| `docs/specs/admin.md` | Drift | Destructive commands work but `--yes` guard missing. |
...
All 66 ADRs in numeric order. Status legend: **Done**, **Partial**, **Drift** (code differs from ADR by intent), **Deferred** (post-MVP per ADR 0066 or phases.md), **Superseded**.
...
| 0024 treat agent data as untrusted | Partial | CSP + private R2 + signed URLs in place. Final security pass against ADR 0029 pending. |
...
| 0039 authenticated rate limits | Partial | Bindings declared in `wrangler.jsonc`. **No `.limit()` call exists in any app source.** Enforce or explicitly defer. |
...
| 0042 strict extension content type | Partial | Content type derived from extension. Confirm no user-supplied MIME override path. |
...
| 0046 operator identity + admin surface | Drift | Single bearer admin token (`ADMIN_TOKEN_HASH`) used instead of Cloudflare Access + email allowlist. Acceptable for MVP per ADR 0066; amend or write follow-up ADR. |
...
| 0064 native rate-limit bindings | Partial | Same gap as ADR 0039. |
...
| Phase 2 — Admin ergonomics | Admin CLI polish, observability depth | 0% | Logpush → Axiom; CLI `--yes` guards; richer event browser; rotation tooling (ADR 0045). |
...
### 3. Add `--yes` guards to destructive admin CLI commands
...
- Done: only one of `CONTENT_SIGNING_SECRET` / `CONTENT_GATEWAY_SIGNING_KEY_V1` remains, named consistently across code, bootstrap script, and ADRs; a one-time rotation note added to `docs/ops/runbook` (or this doc) for any environment that already holds both.
...
- Done: a same-repo PR (the one carrying items 1-8 above is the natural candidate) creates a Neon branch, deploys preview Workers, runs hosted smoke, posts a comment with URLs, and tears everything down on close. Captured run links recorded in this doc.

### 10. Wire Logpush to Axiom for `api`/`upload`/`content`
...
### 12. Decide on remaining ADR 0046 drift (operator identity)
- Drives: ADR 0046
...
### 14. Complete bootstrap hosting checklist
