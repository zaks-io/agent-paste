...
- Coordination note: as of 2026-05-24 no other worktree is active, so this lane is clear to proceed. The ADR 0072 registrar work deliberately rewires the `api` web-route plumbing (`withWebMember`, `web.auth.callback`); `apps/web` itself is untouched. Restore the avoid-list (`apps/web`, Auth0, Access Links, dashboard implementation) if a parallel app-worker lane restarts.
...
- Public Agent View accepts only signed Agent-View Tokens; there is no unsigned `{artifactId}.{revisionId}` fallback.
...
| ------------------------- | ---------------------- | ---------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/api`                | Implemented            | ~950       | Yes   | `src/index.ts`. Hono routing, `/openapi.json`, public Agent View, admin routes, scheduled cleanup, signed content URLs, denylist writes, WorkOS web callback member provisioning, first dashboard read APIs (`/v1/web/workspace`, `/v1/web/artifacts`, `/v1/web/artifacts/{artifactId}`), dashboard API key lifecycle, and cursor-paginated audit reads.                                                                                                                                                                                                                                                                                                 |
| `apps/upload`             | Implemented            | ~14k       | Yes   | `src/index.ts`. Session create, signed PUT, R2 writes, finalize, signed Agent View URL minting.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `apps/content`            | Implemented            | ~14k       | Yes   | `src/index.ts`. Signed content URL verification, CSP, extension content-type, KV denylist, and artifact-level read throttling.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `packages/contracts`      | Implemented            | ~810       | Yes   | Zod schemas, branded IDs, load-bearing route registry with auth/scope/idempotency/rate-limit declarations. CLI-first MVP surface only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `packages/worker-runtime` | Implemented            | ~400       | Yes   | Contract-driven Hono route registrar, request guard, principal model, centralized error status map, and rate-limit application for `api`, `upload`, and `content`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `docs/specs/mvp.md`               | Partial | Publish/read flows work; delete/expiry byte-purge and artifact read throttling are verified in local/hosted coverage. Remaining MVP-shaped hardening is interim operator identity follow-ups. Rotation has an MVP runbook; automation remains future work.                                                                                 |
...
| `docs/specs/features.md`          | Partial | MVP CLI + 3 Workers implemented, including artifact read caps. Future features (multi-revision, OAuth, MCP, dashboard UI wiring) are absent by design.                                                                                                                                                                                     |
...
| `docs/specs/content-rendering.md` | Done    | Content Worker handles signed tokens, strict extension MIME, denylist, CSP, SVG strict-CSP, CSP snapshots, and artifact-level read throttling.                                                                                                                                                                                             |
...
| `docs/specs/acceptance.md`        | Partial | Local/hosted smoke now cover publish/read/delete/expiry byte-purge, Agent View, operation events, authenticated rate-limit 429s, and unit coverage for artifact read throttling. Remaining gap is broader preview/load coverage.                                                                                                           |
...
| `docs/specs/web.md`               | Partial | TanStack Start scaffold implements every route from the spec with WorkOS AuthKit + sealed sessions per ADR 0068. `apps/api` now serves workspace/artifact reads, API key lifecycle, and cursor-paginated audit reads; settings, admin lockdown, Access Links, and loader replacement remain -- see [`web-app-todo.md`](./web-app-todo.md). |

## ADR Coverage

All 72 ADRs in numeric order. Audit on 2026-05-23 confirmed every `docs/adr/0001` through `0068` file has a row here; ADR 0069 (Live Updates) and ADR 0070 (repository core) were added 2026-05-23, and ADR 0071 (signed-token codec) and ADR 0072 (contract-driven route registrar) were added 2026-05-24, all recorded below; rows 0029/0030 were corrected to match the ADR filenames. Status legend: **Done**, **Partial**, **Drift** (code differs from ADR by intent), **Deferred** (post-MVP per ADR 0066 or phases.md), **Superseded**.
...
| 0033 TanStack Start web app               | Partial      | Scaffold landed: TanStack Start on Cloudflare Workers via `@cloudflare/vite-plugin`, file routes, sealed `__agp_session` cookie, WorkOS AuthKit via `@workos/authkit-tanstack-react-start` (ADR 0068), service binding to `api`. Workspace/artifact reads, key lifecycle, and audit pagination endpoints exist; remaining loaders fall back to `EmptyState` until their endpoints land. Tracked in [`web-app-todo.md`](../ops/web-app-todo.md).             |
...
| 0048 transient artifacts by default       | Partial      | TTL defaults, delete/expiry stop-serving behavior, R2 byte purge, denylist writes, and artifact-level unauthenticated read throttling are verified. Pinning remains absent/deferred.                                                                                                                                                                                                                                                                        |
...
| 0055 signup auto-provisions workspace     | Partial      | `POST /v1/auth/web/callback` now provisions a Personal Workspace, Workspace Member, and default API Key on first WorkOS AuthKit login. `/v1/web/workspace` surfaces the Workspace, Workspace Member, Usage Policy, and first-run default-key flag. Dashboard API key lifecycle and cursor-paginated audit reads are implemented. Remaining Phase 3 work is real WorkOS project/secret setup, settings/admin endpoints, Access Links, and web loader wiring. |
...
| 0059 web app session sealing              | Partial      | Sealed `__agp_session` (HttpOnly, Secure, SameSite=Lax, no `Domain`) owned by WorkOS AuthKit (iron-session blob) per ADR 0068; PKCE state handled by AuthKit internally; `api` verifies forwarded WorkOS access tokens with JWKS + `client_id`. Current secret name is `WORKOS_COOKIE_PASSWORD`; `WEB_SESSION_SEAL_KEY_V1` is intentionally excluded from MVP rotation/bootstrap.                                                                           |
...
| 0064 native rate-limit bindings           | Done         | Bindings are called in `api`, `upload`, and unauthenticated `content` reads; upload routes peek idempotency before rate-limit; hosted smoke asserts the per-actor 429.                                                                                                                                                                                                                                                                                      |
...
| 0071 signed-token codec / packages/tokens | Done         | Implemented 2026-05-24. `@agent-paste/tokens` owns the `base64url(payload).hmac` codec and the three token-kind modules; `packages/auth`, `packages/db`, and the 3 Workers import it and deleted their HMAC/base64/constant-time copies. Refines ADR 0028. See [`signed-tokens-todo.md`](./signed-tokens-todo.md).                                                                                                                                          |
| 0072 contract-driven route registrar      | Done         | `@agent-paste/worker-runtime` mounts `api`, `upload`, and `content` route contracts through one registrar and request guard. Route Contracts now carry `rateLimit`; the inlined api-key chains, upload/content rate-limit helpers, and `withWebMember` are deleted.                                                                                                                                                                                         |
...
- Drives: ADR 0072; ADR 0034/0035/0036/0039/0064 (preserved).
- Files: `packages/worker-runtime/*` (new), `packages/contracts/src/routes.ts`, `apps/api/src/index.ts`, `apps/upload/src/index.ts`, `apps/content/src/index.ts`, focused Worker/runtime tests, and [`worker-runtime-todo.md`](./worker-runtime-todo.md).
- Done: every contract route in `api`, `upload`, and `content` mounts through `createRegistrar(deps).mount(contract, handler)`. The inlined api-key chains, upload/content rate-limit helpers, and `withWebMember` are deleted; `routeContracts` now declares `rateLimit`; authenticated Agent View routes were added to the registry; `ERROR_STATUS` is centralized and tested against every `ErrorCode`. Focused tests, OpenAPI check, equivalent full verification, and local smoke are green.

### Extract the signed-token codec into `packages/tokens`

- Status: Done on 2026-05-24.
- Drives: ADR 0071; ADR 0028 and ADR 0015 (preserved).
- Files: `packages/tokens/*` (new), `packages/auth/src/index.ts`, `packages/db/src/{api-keys,id}.ts`, `apps/api/src/index.ts`, `apps/upload/src/index.ts`, `apps/content/src/index.ts`. Steps in [`signed-tokens-todo.md`](./signed-tokens-todo.md).
- Done: `@agent-paste/tokens` owns the `base64url(payload).hmac` codec and the three token-kind modules (`content`, `agent-view`, `upload-url`). `packages/auth`, `packages/db`, and all three Workers import `./crypto` and deleted their HMAC/base64/constant-time copies (grep clean; only the standalone deploy scripts retain `hmacBase64Url` on purpose). Scope grew beyond the ADR to fold in the two `packages/db` copies ADR 0071 missed, and auth's dead test-only api-key trio was deleted so `db` is the sole canonical api-key home. The three ADR 0071 reconciliations landed: non-short-circuiting `constantTimeEqual`, non-throwing `verify`, explicit per-caller `exp`. `pnpm verify` (66 Turbo tasks) and `pnpm smoke:local` green.

### Add artifact-level read throttling on `content`

- Status: Done on 2026-05-23.
- Drives: ADR 0048, `docs/specs/content-rendering.md`, Security Pass follow-up.
- Files: `apps/content/wrangler.jsonc`, `apps/content/src/index.ts`, `apps/content/src/worker-configuration.d.ts`, content tests, `packages/contracts/src/common.ts`, `packages/contracts/src/routes.ts`, content OpenAPI, `packages/auth/src/request-id.ts`.
- Done: signed content reads check token/path validity and the KV denylist before consuming the native `ARTIFACT_RATE_LIMIT` budget keyed by Artifact. Over-limit reads return 429 with `rate_limited_artifact`, `Retry-After: 60`, and `X-Request-Id`. Missing local bindings are allowed; binding errors fail open with a warning. Tests cover allowed reads, limited reads, denied reads, invalid token/path reads, and binding failures.

### Add dashboard key lifecycle and audit pagination APIs

- Status: Done on 2026-05-23.
- Drives: ADR 0035, ADR 0044, ADR 0055, ADR 0068; `docs/specs/web.md`.
- Files: `apps/api/src/index.ts`, `apps/api/src/index.test.ts`, `apps/cli/src/index.ts`, `packages/commands/src/index.ts`, `packages/contracts/src/{apiKeys,enums,routes}.ts`, `packages/contracts/src/openapi/api.ts`, `packages/contracts/openapi/api.json`, `packages/db/migrations/0005_member_actor_events.sql`, `packages/db/migrations/0006_operation_events_web_audit_cursor_index.sql`, `packages/db/snapshot/schema.sql`, `packages/db/src/{index.test,types,schema}.ts`, `packages/db/src/repository/*`, `packages/db/src/postgres/rls.test.ts`, `packages/db/src/queries/operation-events.ts`, `docs/ops/web-app-todo.md`.
- Done: `GET /v1/web/keys` remains implemented and `POST /v1/web/keys` plus `POST /v1/web/keys/{api_key_id}/revoke` now authenticate WorkOS Workspace Members, require `admin` scope and idempotency keys, derive the Workspace from the member, write `member` audit/idempotency rows through `runCommand`, and return generic `not_found` for missing or cross-workspace revokes. `GET /v1/web/audit` now accepts `limit` and `cursor`, orders by `occurred_at desc, id desc`, and returns cursor-paginated Workspace-scoped Audit Events.
...
- Status: Done on 2026-05-23.
- Drives: ADR 0044, ADR 0055, ADR 0068; `docs/specs/web.md`.
- Files: `apps/api/src/index.ts`, `packages/contracts/src/routes.ts`, `packages/contracts/src/openapi/api.ts`, `packages/contracts/openapi/api.json`, `packages/db/src/{local-repository,index.test}.ts`, `packages/db/src/postgres/repository.ts`, `packages/db/src/queries/artifacts.ts`, `apps/api/src/index.test.ts`, `docs/ops/web-app-todo.md`.
- Done: `GET /v1/web/workspace` authenticates WorkOS access tokens through the Workspace Member path from the callback API and returns Workspace, Workspace Member, Usage Policy, and `default_key_first_run`. `GET /v1/web/artifacts` runs under the member workspace RLS scope and returns cursor-paginated artifact rows. `GET /v1/web/artifacts/{artifactId}` also runs under the member workspace scope and returns a generic `not_found` envelope for missing or cross-workspace artifacts. This tranche originally left keys, audit, settings, and admin lockdown deferred; keys and audit are now covered by the later completed API tranche above.
...
### Implement WorkOS web auth callback API
