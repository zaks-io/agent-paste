...
Last updated: 2026-05-24 (locked two deepening refactors: ADR 0071 signed-token codec / `packages/tokens` and ADR 0072 contract-driven route registrar / `packages/worker-runtime`, both accepted and pending implementation per [`signed-tokens-todo.md`](./signed-tokens-todo.md) and [`worker-runtime-todo.md`](./worker-runtime-todo.md). Prior: artifact-level content read throttling; dashboard API key lifecycle and cursor-paginated audit reads; repository adapters unified behind a backend-agnostic `RepositoryCore` per ADR 0070; Logpush and production deploy-gate work parked for later).
...
| 0071 signed-token codec / packages/tokens | Accepted (pending) | New 2026-05-24. `@agent-paste/tokens` not yet created; `packages/auth` + the 3 Workers still carry duplicate HMAC/base64/constant-time copies. Refines ADR 0028. Implementation in [`signed-tokens-todo.md`](./signed-tokens-todo.md).                                                                                                                                                                                                                      |
