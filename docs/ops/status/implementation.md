...
| `apps/jobs`               | Partial                     | Cron discovery (upload cleanup, auto-deletion skipping pinned artifacts, retention for non-current revisions when `revision_retention_days` is set — `rd:` denylist keys plus revision-scoped `byte-purge` enqueue before `retained` status — purge recovery, maintenance GC), queue consumers + DLQs, bundle zip generation + DLQ `mark_failed`, safety-scan stub (AP-21/22/23/24). |
| `apps/mcp`                | Partial                     | Health/OpenAPI, OAuth protected-resource metadata, and Streamable HTTP JSON-RPC transport with stateless bearer auth hooks and `WWW-Authenticate` challenges. WorkOS JWT verification, API forwarding, and tool execution are not implemented yet.                                                                                                                                   |
...
| `packages/db`             | Implemented for current app | Drizzle schema/migrations, RLS, repository core/adapters, `revisions` table with bundle availability columns and publish-update flow. AP-19 Access Link model/codec/mint-revoke persistence landed; billing/scanner persistence still absent.                                                                                                                                        |
...
| Jobs queues        | Phase 4        | `apps/jobs` has Wrangler queue/DLQ bindings, consumers, lifecycle sweeps, and `bundle-generate` + DLQ consumers (AP-21–23). Pinning/retention policy lands in AP-24 (PR #104).                                         |
...
- `apps/jobs` owns lifecycle byte purge and retention sweeps (AP-22) and bundle zip
  generation + DLQ terminal failure handling (AP-23).
- `apps/mcp` implements MCP JSON-RPC transport and bearer auth hooks but does not
  verify WorkOS JWTs, forward to `api`, or execute tools yet.
...
- `packages/db` has workspace/member/key/artifact/revision/audit/lockdown state
  ...
  revisions; billing and scanner persistence remain absent.
