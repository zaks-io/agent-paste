...
| `apps/jobs`               | Partial                     | Cron discovery (upload cleanup, auto-deletion skipping pinned artifacts, retention for non-current revisions when `revision_retention_days` is set, purge recovery, maintenance GC), queue consumers + DLQs, bundle zip generation + DLQ `mark_failed`, safety-scan stub (AP-21/22/23/24).                         |
