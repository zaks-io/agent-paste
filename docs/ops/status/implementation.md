...
- AP-99 through AP-111 add Ephemeral Workspace provisioning, claim, CLI
  `--ephemeral`, web claim UX, `noindex` serving, and local +
  hosted ephemeral smokes. Operator notes:
  [`runbook-ephemeral-publish.md`](../runbook-ephemeral-publish.md).
- Retired snapshot: AP-156 shipped dashboard Access Link management (member
  `/v1/web/*` routes plus list/create/mint/revoke/lockdown UI); AP-164 added the
  TanStack Query client cache and now-retired SSE-driven live dashboard.
  AP-161/AP-162 make artifact TTL
...
| `apps/content`            | Implemented                 | Signed content-token verification, durable per-Artifact capability manifests, private R2 reads/decrypt, open artifact CSP/security headers, noindex ephemeral serving, extension-derived MIME, denylist, read throttling.                                                                                                                                   |
...
| `apps/web`                | Snapshot; viewer retired    | WorkOS AuthKit, dashboard routes, live loaders/mutations, operator lockdown UI, operator event browsing, Lighthouse a11y gate, hardened PR-preview readiness, deployed preview/production. The Access Link `/al/{publicId}` viewer, resolve proxy, and management UI described by this snapshot are retired.                                                |
...
| `apps/stream`             | Retired                     | Retired Live Updates Worker (ADR 0069): per-artifact Durable Objects, SSE fan-out, `stream -> api` authorize binding, viewer cap (AP-25).                                                                                                                                                                                                                   |
| `packages/contracts`      | Implemented for snapshot    | Zod schemas, route registry, OpenAPI goldens, and the now-retired Access Link resolve contracts. MCP OAuth scopes, JSON-RPC transport shapes, fourteen-tool registry, error mapping, and forwarded API call plans (AP-27).                                                                                                                                  |
...
| `packages/db`             | Implemented for snapshot    | Drizzle schema/migrations, RLS, repository core/adapters, `revisions` table with bundle availability columns and publish-update flow. Retired Access Link tables remain alongside Safety Warnings, `workspace_billing`, `claim_tokens`, and Ephemeral Workspace provisioning.                                                                               |
| `packages/tokens`         | Implemented for snapshot    | Shared signed-token codec and content, Agent View, upload URL, and retired Access Link modules.                                                                                                                                                                                                                                                             |
...
| Access Link management UI       | Phase 4        | Retired (was Done in AP-156): `/v1/web/*` member routes + dashboard list/create/mint/revoke/lockdown UI on `/access-links` and artifact detail.                                               |
...
- `packages/db` has workspace/member/key/artifact/revision/audit/lockdown state
  plus retired Access Link rows, bundle availability, Safety Warning persistence,
