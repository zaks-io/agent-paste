...
| `packages/billing`        | Implemented                 | Plan tiers, plan-derived caps, the Stripe `BillingProvider` (Checkout, idempotent webhooks, Portal, operator override, invoices, AP-5), sync/reconciliation, and drift logging. Billing routes/Stripe import only when `BILLING_ENABLED` is set. The `/settings/billing` dashboard ships in `apps/web` (AP-176).                                                                                    |
...
| Hosted billing UI               | Post-launch    | Done (AP-176): `/settings/billing` dashboard with plan, subscription, live allowance, and Stripe invoices.                       |
