...
| Jobs queues        | Phase 4        | `apps/jobs` has Wrangler queue/DLQ bindings, consumers, lifecycle byte purge/retention sweeps, and `bundle-generate` + DLQ consumers (AP-23). Pinning/non-current retention policy remains AP-24.                      |
