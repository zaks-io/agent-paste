...
- [x] **Add `rateLimit` to the Route Contract.** `"none" | "actor" | "artifact"` on
...
- [x] **`registrar.ts`.** `createRegistrar(deps)` with `AuthResolvers`, `.mount(contract, handler)`,
  ...
  - Done: `pnpm --filter @agent-paste/worker-runtime check` green; a fakes-based suite drives the
    ...
    no Hono server.
- [x] **Cut over `apps/content`.** Mount `content.get` / `content.head` through the registrar with
...
- [x] **Cut over `apps/upload`.** Mount the upload routes; delete the inlined
...
- [x] **Cut over `apps/api`.** Mount all `api` routes; delete the three inlined api-key chains and
