...
| `node scripts/deploy.mjs <local\|preview\|production>` | Migrate (when a DB-backed app is in scope), provision secrets, then build + deploy via Turbo. `--app=<name>` scopes it; `--no-migrate` skips migrations. `local` writes a gitignored `.env`. Idempotent; never prints a value. |
