...
| [`apps/api`](../apps/api)                                 | Implemented | Authenticated control-plane Worker, capability-manifest writer, dashboard API, admin/operator routes, and cleanup scheduler.                                                     |
| [`apps/upload`](../apps/upload)                           | Implemented | Upload-session Worker, signed upload-worker PUT URLs, private R2 writes, finalize flow.                                                                                          |
| [`apps/content`](../apps/content)                         | Implemented | Isolated untrusted-content Worker serving each Artifact top-level on its unguessable capability subdomain, with R2 reads, denylist checks, CSP/header hardening, and throttling. |
...
| [`apps/web`](../apps/web)                                 | Implemented | TanStack Start dashboard Worker with WorkOS AuthKit and dashboard loaders/mutations. Artifact links open their capability subdomain directly.                                    |
| [`apps/jobs`](../apps/jobs)                               | Implemented | Queue/cron Worker for bundle generation, byte purge, safety scan, and lifecycle discovery.                                                                                       |
| [`apps/stream`](../apps/stream)                           | Dormant     | Retained migration history for the removed iframe viewer's Live Update transport; no current API or web route consumes it.                                                       |
| [`apps/mcp`](../apps/mcp)                                 | Implemented | OAuth-only hosted MCP Worker: Streamable HTTP transport, WorkOS bearer verification, and ten publish/read/edit tools over service bindings to `api` and `upload`.                |
...
| [`packages/auth-md`](../packages/auth-md)                 | Implemented | The auth.md agent-registration skill document and its protocol URNs, shared by `apps/api` and `apps/apex` so both origins serve one text.                                        |
...
| [`packages/api-client`](../packages/api-client)           | Implemented | Internal HTTP client used by the CLI and shared revise engine.                                                                                                                   |
| [`packages/storage`](../packages/storage)                 | Implemented | Artifact-byte encryption and R2 helpers, reparenting, unified-diff application, content types, and security headers.                                                             |
...
| [`packages/tokens`](../packages/tokens)                   | Implemented | Shared signed-token codec and token-kind modules for capability manifests, legacy content reads, Agent View, and upload URLs.                                                    |
| [`packages/rotation`](../packages/rotation)               | Implemented | Tested multi-key and multi-pepper rotation rings and overlap playbooks.                                                                                                          |
| [`packages/config`](../packages/config)                   | Implemented | Shared limits, Plan usage policy, write allowances, billing gates, storage-path normalization, and expiration helpers.                                                           |
...
| [`packages/plans`](../packages/plans)                     | Implemented | Presentational Plan definitions for the billing dashboard and billing-enabled apex pricing page.                                                                                 |
...
| `pnpm mutation:tokens`          | Advisory StrykerJS mutation pilot for `packages/tokens`. Local-only, not in `verify`/CI. See [mutation-testing.md](ops/mutation-testing.md).                                                                                                                                                      |
| `pnpm mutation:revise-core`     | Advisory StrykerJS mutation pilot for `packages/revise-core`. See [mutation-testing.md](ops/mutation-testing.md).                                                                                                                                                                                 |
| `pnpm mutation:storage`         | Advisory StrykerJS mutation pilot for `packages/storage`. See [mutation-testing.md](ops/mutation-testing.md).                                                                                                                                                                                     |
| `pnpm mutation:billing`         | Advisory StrykerJS mutation pilot for `packages/billing`. See [mutation-testing.md](ops/mutation-testing.md).                                                                                                                                                                                     |
| `pnpm mutation:rotation`        | Advisory StrykerJS mutation pilot for `packages/rotation`. See [mutation-testing.md](ops/mutation-testing.md).                                                                                                                                                                                    |
| `pnpm mutation:write-allowance` | Advisory StrykerJS mutation pilot for `packages/write-allowance`. See [mutation-testing.md](ops/mutation-testing.md).                                                                                                                                                                             |
...
| `pnpm depcruise`                | Validate the module dependency graph with dependency-cruiser: no import cycles, no devDependency leakage into shipped code, and the app/Worker trust boundaries in `docs/specs/architecture.md`. Rule baseline and ratchet plan: [`docs/ops/dependency-cruiser.md`](./ops/dependency-cruiser.md). |
...
| `pnpm verify`                   | Full CI-style local verification: code + docs format checks, Knip, duplication, dependency-graph check, lint, typecheck, tests, scripts typecheck, OpenAPI check, and DB check.                                                                                                                   |
...
| `pnpm secrets:rotate:workos-api-key:preview`           | Write `WORKOS_API_KEY` to preview `api`, `mcp`, `upload`, then `web` (requires `--value-env`; do not pass secret material through pnpm argv).                                                                                                                                   |
...
dependency-graph, lint, typecheck, tests, OpenAPI, and DB checks) plus `pnpm test:coverage:strict`
