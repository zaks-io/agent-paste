...
| `pnpm secrets:rotate:workos-api-key:preview`           | Write `WORKOS_API_KEY` to preview `api` then `web` (requires `--value-env`; do not pass secret material through pnpm argv).                                                                                                                                                     |
