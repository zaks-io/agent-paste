...
| `patch_result_sha256` | `TEXT NULL`                                    | Digest of the whole reconstructed file `jobs` produces from the diff (Stage 4). Both patch columns are `NULL` (whole-file upload) or both set (a `CHECK` enforces it).                    |
