...
- Public auth: CLI credential only.
...
- Output: human-facing publish prints the authenticated Artifact URL as `View`; CLI JSON output includes `artifact_id`, `revision_id`, `artifact_url`, optional `access_link_url`, direct signed `revision_content_url`, public signed `agent_view_url`, and `expires_at`. `access_link_url` appears during publish only when the caller explicitly requests sharing (`--share` or MCP `share:true`).
...
- Routine hosted operations can be handled through the public CLI, smoke harness, and Codex-assisted runbooks without changing the CLI-first product shape.
...
- WorkOS AuthKit web sign-in/signup ([ADR 0068](../adr/0068-workos-authkit-for-web-app-auth.md)); first login auto-provisions a Personal Workspace and Workspace Member.
- Web dashboard: workspace overview, artifact list/detail, credential management, audit log, and workspace usage/retention settings (retention bounded by the ADR 0048 caps).
...
- `agent-paste login` stores a scoped local credential; hosted agents without CLI access use MCP.
