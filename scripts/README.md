...
- It then hands build + deploy to Turbo (`turbo run deploy:<target>`, which dependsOn `build`), so every workspace dependency is built in graph order (cached) before `wrangler deploy` runs. The content Worker deploys and passes its public readiness check before API and upload can mint URLs on its domain. Routing (which secret binds to which Worker) is the single source of truth in `lib/secret-routing.mjs`, and the same data backs each Worker's `secrets.required` in `wrangler.jsonc`, so a missing required secret fails the deploy.
