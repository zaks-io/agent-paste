...
- `--app=<name>` (comma-separated) scopes both provisioning and the Turbo deploy to one Worker (e.g. `--app=apex` for the marketing page); `--no-migrate` skips migrations. Migrations run automatically only when a DB-backed Worker (`api`, `upload`, `jobs`) is in scope.
