...
import { type SqlExecutor, withSqlQuerySource, writeArtifactDenylist } from "@agent-paste/db";
...
export async function applyMaliciousUrlLockdown(
  ...
  const command = await withSource("applyMaliciousUrlLockdown", () =>
    runCommand({
...
function withSource<T>(functionName: string, run: () => T): T {
  return withSqlQuerySource(
    {
      filepath: "apps/jobs/src/safety/platform-lockdown.ts",
      functionName,
      namespace: "apps.jobs.src.safety.platform-lockdown",
    },
    run,
  );
}
