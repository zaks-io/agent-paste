...
| Maintenance GC    | `0 * * * *`    | Deletes aged `idempotency_records`; archives expired `operation_events` to R2 (`audit/` NDJSON) then deletes from Postgres (no `runCommand`).                                                                                                                                          |
...
## Ephemeral safety scanner binding
...
URL Scanner is paused until scale justifies its operating cost. Built-in and Llama Guard
results remain advisory; operators apply Platform Lockdown through the admin surface.
