...
export function StatBand({ stats, className }: { stats: ReadonlyArray<Stat>; className?: string }) {
  return (
    <div className={cn("grid grid-cols-2 border-y border-rule sm:grid-cols-4", className)}>
      {stats.map((stat, i) => (
        <div
          key={stat.label}
          className={cn("flex flex-col gap-2 py-5 pr-6", i > 0 && "sm:border-l sm:border-rule sm:pl-6")}
        >
          ...
          {stat.hint ? <span className="font-mono text-mono-sm text-subtle">{stat.hint}</span> : null}
        </div>
      ))}
    </div>
  );
