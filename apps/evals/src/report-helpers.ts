export function failureSuggestedFix(failure: string): string | undefined {
  if (failure === "missing_final_answer_artifact_url") {
    return "Require the agent's final answer to include the clean artifact_url, not only raw publish JSON or tool output.";
  }
