...
describe("content worker", () => {
  ...
  it("denies iframe rendering even when the request came from the app", async () => {
    const token = await signContentToken(
      {
        workspace_id: workspaceId,
        artifact_id: "art_1",
    ...
    const response = await handleRequest(
      new Request(`https://content.test/v/${token}/index.html`, {
        headers: { "sec-fetch-dest": "iframe", "sec-fetch-mode": "navigate", "sec-fetch-site": "same-site" },
      }),
      env,
    );
...
describe("CSP header per content type", () => {
  ...
  it("keeps explicitly interactive legacy HTML top-level and frame-denied", async () => {
    const response = await fetchServedFile("index.html", "ok", { script_disabled: false });
