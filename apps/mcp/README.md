...
- Text-only MCP tool surface (fourteen tools).
...
bearer. The current fourteen-tool surface is defined in
...
| `set_visibility`          | `publish, read` | Set visibility: `private` revokes active Access Links; `unlisted` returns `unlisted_url`. |
...
make an Artifact reachable without login, call `set_visibility` with
