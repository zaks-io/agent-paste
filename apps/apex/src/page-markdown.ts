...
export function renderHomeMarkdown(billingEnabled: boolean): string {
  return lines([
    ...
    "",
    ...fence("sh", `${LOGIN_CMD}\n${PUBLISH_CMD}`),
    "In a sandbox or SSH session, use device login and keep it running while the human approves the URL and code in their own browser:",
    "",
    ...fence("sh", `${LOGIN_CMD} --device-code`),
    "After login succeeds, run `agent-paste whoami --json` before publishing. Device login needs network access to WorkOS and the API, but no local browser. See [CLI authentication](/docs/cli#remote-login).",
    "",
